...
import { Plus, Trash2, Video, Wand2, Image as ImageIcon, ChevronDown, ChevronUp, Upload, X, LayoutDashboard, Copy, ExternalLink, FileVideo, User, MessageSquare, Loader2, CheckCircle2, Globe, Crop, Settings, LogOut, AlertCircle, Share2, Eye, EyeOff, Cloud, Link, Sparkles, Wrench, ZoomIn, ZoomOut, QrCode, Download, AlertTriangle, RefreshCw } from 'lucide-react';
import Cropper from 'react-easy-crop';
import { getCroppedImg, generateThumbnailFromVideo, encodeState, uploadFileToStorage, isConfigured, hasCloudStorage, generateAiBio, generateAiDescription, checkPortfolioReadiness, downloadQrCode, getYouTubeThumbnail, getYouTubeId } from '../utils';
...
export const EditorPanel: React.FC<EditorPanelProps> = ({ data, onChange, onPublish, isSaving, hasUnsavedChanges, onLogout, onPreview }) => {
  ...
  // Ref to hold the latest data to prevent stale closures in async upload handlers
  ...
  const handleAiBio = async () => {
      // Validation: User must type something first
  ...
  // Short URL for display purposes
  ...
  const saveCroppedImage = async () => {
      ...
      try {
          ...
          if (hasCloudStorage) {
             ...
             // Use ref to ensure we don't overwrite other changes
  ...
  const handleShowreelVideoUpload = async (file: File) => {
    ...
    if (hasCloudStorage) {
        try {
            ...
            // Use ref to ensure we don't overwrite
  ...
  const handleProjectVideoUpload = async (id: string, file: File) => {
    ...
    if (hasCloudStorage) {
        try {
            ...
            // Fetch latest data state from ref to avoid stale closures
            ...
            if (generatedThumb) {
                ...
                finalProjects = finalProjects.map(p => p.id === id ? { 
                    ...p, 
                    thumbnail: thumbUrl,
  ...
  const handleLoadVideoLink = (type: 'showreel' | 'project', link: string, projectId?: string) => {
      ...
      // Trigger a save after loading link
  ...
  return (
    <div className="h-full flex flex-col bg-zinc-950 border-r border-zinc-900 relative">
      ...
      {/* Background Pattern - Cool Gradient Doodle */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
          ...
          <div className="absolute top-[40%] right-[30%] w-[20vw] h-[20vw] bg-blue-900/10 rounded-full blur-[60px] animate-pulse delay-500"></div>
                  ...
                  <div className="p-6 bg-zinc-950 border-t border-zinc-800 flex flex-col gap-6 safe-area-pb relative z-10">
                     ...
                     <div className="flex gap-3">
                         ...
                         <Button variant="primary" className="flex-1 py-3" onClick={saveCroppedImage} icon={profileImageUploading ? <Loader2 className="animate-spin" size={14}/> : <Crop size={14}/>}>
                            {profileImageUploading ? 'Processing...' : 'Save Photo'}
                         </Button>
                     </div>
                  </div>
              </div>
      ...
      <div className="p-6 border-b border-zinc-900 bg-zinc-950/80 backdrop-blur-md flex justify-between items-center sticky top-0 z-50">
        ...
        <div className="flex items-center gap-3">
             <Button 
                ...
                size="sm" 
                onClick={onPublish}
                className="hidden sm:flex shadow-xl"
            >
            ...
            <div className="relative group">
                <Button 
                    ...
                    onClick={onPreview}
                    disabled={isLocked}
                    className={`border-zinc-700 bg-zinc-900 ${isLocked ? 'opacity-50 cursor-not-allowed' : 'hover:border-zinc-500'}`}
                    icon={<Eye size={16}/>}
                >
                    Preview
                </Button>
                {isLocked && (
                    <div className="absolute top-full right-0 mt-2 w-64 bg-zinc-950 border border-red-500/30 rounded-xl p-3 shadow-2xl opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50">
                        <div className="text-xs text-red-300">
                           {hasUnsavedChanges ? "Please save changes first." : "Complete required fields."}
                        </div>
                    </div>
                )}
            </div>
      ...
      <div className="flex border-b border-zinc-900 overflow-x-auto no-scrollbar bg-zinc-950/80 backdrop-blur-md flex-shrink-0">
        {[
           ...
           className={`flex-1 py-4 px-3 text-xs font-medium transition-all flex flex-col items-center gap-1 min-w-[80px] relative hover:bg-white/5 ${activeTab === tab.id ? 'text-white' : 'text-zinc-500'}`}
         >
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar relative z-10">
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-8 animate-fadeIn">
             ...
             {/* VALIDATION CARD: Only show if NOT ready */}
             {!isReady && (
             ...
             <div className="bg-zinc-900/40 border border-zinc-800 rounded-3xl p-8 space-y-6 backdrop-blur-sm">
                <div className="flex items-center justify-between">
                    <div>
                        <h3 className="text-white font-bold text-lg flex items-center gap-2">WATCH {data.username.toUpperCase()}'S PORTFOLIO</h3>
                        <p className="text-xs text-zinc-500 mt-1">
                            {isLocked 
                                ? <span className="text-red-400 font-medium">Link inactive. Complete setup & save changes first.</span> 
                                : "Share this link with clients or on social media."}
                        </p>
                    </div>
                ...
                <div className={`bg-black/60 rounded-xl p-4 border flex items-center justify-between gap-4 ${isLocked ? 'border-zinc-800 opacity-50' : 'border-indigo-500/30'}`}>
                   <div className="flex items-center gap-3 text-zinc-500 overflow-hidden flex-1">
                      <Link size={16} className={`flex-shrink-0 ${isLocked ? 'text-zinc-600' : 'text-indigo-500'}`}/>
                      <span className={`text-sm font-mono truncate select-all ${isLocked ? 'text-zinc-600' : 'text-indigo-300'}`}>
                          {getShortDisplayUrl()}
                      </span>
                   </div>
                   <Button 
                        ...
                        icon={copySuccess ? <CheckCircle2 size={14}/> : <Copy size={14}/>}
                    >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <Button 
                        variant="secondary" 
                        className={`w-full py-4 bg-zinc-800/50 ${isLocked ? 'opacity-50 cursor-not-allowed' : ''}`}
                        onClick={openLink} 
                        disabled={isLocked}
                    ...
                    <Button 
                        variant="outline" 
                        className={`w-full py-4 border-zinc-700 hover:bg-zinc-800 ${isLocked ? 'opacity-50 cursor-not-allowed' : ''}`}
          ...
          <div className="space-y-8 animate-fadeIn">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-8 p-8 bg-zinc-900/40 rounded-3xl border border-zinc-800 backdrop-blur-sm">
                <div className="w-32 h-32 rounded-full bg-zinc-800 overflow-hidden border-4 border-zinc-800 relative group flex-shrink-0 shadow-2xl">
                  ...
                  <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer" onClick={() => handleFileUpload('image/*', handleProfileImageUpload)}>
                ...
                <div className="flex-1 text-center sm:text-left space-y-4">
                    <div>
                        ...
                        <p className="text-xs text-zinc-500">This will be the face of your portfolio. Use a professional headshot.</p>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-6">
                   ...
                   <div className="relative h-full">
                     ...
                     <button 
                        onClick={handleAiBio} 
                        disabled={!data.bio || data.bio.length < 5 || isGeneratingBio}
                        className={`absolute bottom-3 right-3 text-[10px] uppercase font-bold flex items-center gap-1 bg-indigo-500/10 px-2 py-1 rounded transition-colors ${!data.bio || data.bio.length < 5 ? 'opacity-50 cursor-not-allowed text-zinc-500' : 'text-indigo-400 hover:text-white cursor-pointer'}`} 
                        title="Refine Bio with Gemini"
                    >
        ...
        {activeTab === 'content' && (
          <div className="space-y-12 animate-fadeIn">
            {/* Showreel - Redesigned to match screenshot */}
            <div className="bg-zinc-900/40 p-8 rounded-3xl border border-zinc-800 space-y-6 backdrop-blur-sm relative overflow-hidden group">
              ...
              {/* Upload Status / Input Area */}
              <div className="relative">
                  {/* Status Indicator */}
                  {data.showreelLink && (
                      <div className="mb-4 flex items-center justify-between gap-4 w-full">
                          <div className="flex items-center gap-2 text-xs font-bold text-green-500 bg-green-500/10 px-3 py-2 rounded-lg border border-green-500/20 w-fit animate-in fade-in slide-in-from-left-2">
                            <CheckCircle2 size={14} /> 
                            {showreelInputMethod === 'upload' ? "Video Upload Complete" : "Link Active"}
                          </div>
                          {/* Option to replace video if needed, but primary upload area is hidden */}
                          <Button size="sm" variant="outline" className="text-xs h-8 border-dashed" onClick={() => updateField('showreelLink', '')}>
                              Replace Video
                          </Button>
                      </div>
                  )}

                  {/* Hide upload area if video exists */}
                  {!data.showreelLink && (
                    <>
                        {showreelInputMethod === 'upload' ? (
                            ...
                            <div className="h-64 border-2 border-dashed border-zinc-800 bg-black/20 rounded-2xl flex flex-col justify-center p-8 space-y-4 relative">
                                ...
                                <div className="flex gap-2">
                                    <Input value={data.showreelLink} onChange={(e) => updateField('showreelLink', e.target.value)} placeholder="https://youtube.com/..." className="py-4 text-lg bg-black border-zinc-700 focus:border-white" />
                                    <Button variant="secondary" onClick={() => handleLoadVideoLink('showreel', data.showreelLink)}>Load</Button>
                                </div>
                                <p className="text-xs text-zinc-600">Supports YouTube, Vimeo, or direct video links.</p>
                            </div>
                        )}
                    </>
                  )}
                       ...
                       <div className="flex items-center gap-4">
                         ...
                         <div>
                             ...
                             <div className="flex items-center gap-2 text-xs text-zinc-500 mt-1">
                                 ...
                                 <span>•</span>
                                 <span className="bg-zinc-800 px-1.5 py-0.5 rounded text-zinc-400">{project.aspectRatio || '16:9'}</span>
                             </div>
                         </div>
                       </div>
                       <div className="flex gap-2">
                     ...
                     {isExpanded && (
                       <div className="p-6 border-t border-zinc-800 space-y-6 bg-black/20">
                         ...
                         <div className="relative">
                            ...
                            <button 
                                onClick={() => handleAiDescription(project.id, project.title, project.category, project.description)} 
                                className={`absolute top-0 right-0 text-[10px] uppercase font-bold flex items-center gap-1 bg-indigo-500/10 px-2 py-1 rounded transition-colors ${!project.description || project.description.length < 5 ? 'opacity-50 cursor-not-allowed text-zinc-500' : 'text-indigo-400 hover:text-white cursor-pointer'}`}
                                disabled={generatingDescId === project.id || !project.description || project.description.length < 5}
                            >
